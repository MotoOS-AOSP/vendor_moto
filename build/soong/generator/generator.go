...
func init() {
	android.RegisterModuleType("moto_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = motoExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = motoExpandVariables(ctx, dep_file)
	...
	cmd := motoExpandVariables(ctx, String(g.properties.Cmd))
