...
if [ "$1" ]; then
    ...
    if [ -f $file_path ]; then
        ...
        if [[ $file_name == *"OFFICIAL"* ]] || [[ $FORCE_JSON == 1 ]]; then
            ...
            build_type=$(grep ro\.moto\.buildtype ./out/target/product/$DEVICE/system/build.prop | cut -d= -f2);
            base_version=$(grep org\.moto\.version ./out/target/product/$DEVICE/system/build.prop | cut -d= -f2);
            link="https://sourceforge.net/projects/MotoOS/files/${DEVICE}/${file_name}/download"
            echo "{" > $file_path.json
